...
- **Shift**: `morning`, `afternoon`. **Pay type**: `daily`, `monthly`
  (`employees.pay_type`/`payroll_items.pay_type` — `monthly` was removed in
  migration 0016 and restored in 0047; `daily` is the column default).
...
`employee_id → employees (PK)`, `base_salary` (used for `pay_type = 'monthly'`
employees — full monthly pay, attendance-independent), `daily_rate` (used for
`pay_type = 'daily'`), `emergency_contact`, `updated_at`. RLS: Owner / System
...
`id`, `so_number` (unique — the permanent Order Number, auto-assigned once on
insert and never regenerated by anything downstream, e.g. a payment. Format
`ZYS-{YYYY}Y-###` (e.g. `ZYS-2026Y-001`) since Ninth pass, atomic per-calendar-year counter via
`order_number_seq` + `assign_so_number` trigger — resets to `001` each new
year. Orders created before Ninth pass keep their original `SO-YYYY-####`
value forever; `assign_so_number` only sets the number once, on INSERT), `customer_id →
...
`id`, `receipt_number` (unique — the independent Receipt Number, one per
payment, never dependent on `so_number`. Format `ZYS-R-######` since Ninth
pass: a GLOBAL, never-year-resetting counter, `receipt_number_seq` +
`assign_receipt_number` — deliberately does NOT reset per year, unlike
`so_number`/`invoice_number`/`po_number`, per an explicit business decision
to keep receipts continuously sequential. Receipts issued before Ninth pass
keep their original `REC-YYYY-####` value forever),
...
— `'daily'` or `'monthly'`), `days_worked` (snapshot, null for monthly),
`rate` (snapshot of `daily_rate` or `base_salary`), `base_amount` (snapshot —
monthly = `rate`; daily = `round(rate * days_worked, 2)`), `created_at`.
...
- `assign_so_number()` — atomic `ZYS-{YYYY}Y-###` counter (via
  `order_number_seq`), resets per calendar year. Redefined in Ninth pass
  (`0033`) to emit the new format; `sales_order_seq`, the original
  `SO-YYYY-####` counter it superseded, is left in place and dormant —
  every SO created before Ninth pass keeps its original number.
...
- `create_draft_payroll_run(p_period_start, p_period_end, p_pay_date, p_notes)`
  ...
  from `employee_private` (`base_salary` for `pay_type = 'monthly'`) or
  `daily_rate × distinct present/late attendance dates in the period` (for
  `pay_type = 'daily'`), plus period overtime. `SECURITY INVOKER`; RLS on
  `payroll_runs`/`payroll_items`/`employee_private`/`attendance` still
  governs who may call it and what it can see.
- `assign_receipt_number()` — atomic `ZYS-R-######` counter (Seventh pass;
  via `receipt_seq`/`REC-YYYY-####`, resetting per calendar year — redefined
  in Ninth pass, `0033`, to instead draw from `receipt_number_seq`, a
  GLOBAL Postgres sequence that never resets, per an explicit business
  decision. `receipt_seq` is left in place and dormant; every receipt
  issued before Ninth pass keeps its original `REC-YYYY-####` number).
