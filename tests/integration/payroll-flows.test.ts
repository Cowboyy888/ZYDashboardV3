 ...
 *
 * Two pay types (0047 restored the monthly one that 0016 had removed):
 *   - 'daily'   → daily_rate × distinct present/late attendance dates
 *   - 'monthly' → base_salary in full, attendance ignored, days_worked = null
 */
interface Employee {
  id: string;
...
describe('acceptance — monthly-salary pay is fixed regardless of attendance', () => {
  it('pays base_salary in full whether the employee has any attendance or not', () => {
    const employee: Employee = { id: 'm1', payType: 'monthly', baseSalary: 450, dailyRate: null };
    const withAttendance = generateItem(employee, [
      { employeeId: 'm1', businessDate: '2026-07-01', status: 'absent' },
    ]);
...
describe('acceptance — a whole run generates one item per active employee, independently', () => {
  it('mixed monthly/daily employees each compute correctly in the same run', () => {
    ...
    const attendance: AttendanceRecord[] = [
      ...
      // A monthly employee's attendance never affects their pay.
      { employeeId: 'm1', businessDate: '2026-07-01', status: 'absent' },
    ];
